...
//    float* deit_pixel_data_d = deit_preprocess(image_base.pixel_data_d, image_base.source_xsize, image_base.source_ysize, deit_size, corp_size, image_base.channels);
//    float* deit_result_h;
//    deit_result_h = (float*)malloc(corp_size * corp_size * image_base.channels * sizeof(float));
//    cudaMemcpy(deit_result_h, deit_pixel_data_d, corp_size * corp_size * image_base.channels * sizeof(float), cudaMemcpyDeviceToHost);
//    cudaFree(deit_pixel_data_d);
//    std::string deit_output_filename = output_filename_prefix + "_deit.npy";
//    to_npy(deit_output_filename, deit_result_h, corp_size, image_base.channels);
//    free(deit_result_h);
//
//    float* vis_pixel_data_d = vis_preprocess(image_base.pixel_data_d, image_base.source_xsize, image_base.source_ysize, vis_size, image_base.channels);
//    float* vis_reuslt_h;
//    vis_reuslt_h = (float*)malloc(vis_size * vis_size * image_base.channels * sizeof(float));
//    cudaMemcpy(vis_reuslt_h, vis_pixel_data_d, vis_size * vis_size * image_base.channels * sizeof(float), cudaMemcpyDeviceToHost);
//    cudaFree(vis_pixel_data_d);
//    std::string vis_output_filename = output_filename_prefix + "_vis.npy";
//    to_npy(vis_output_filename, vis_reuslt_h, vis_size, image_base.channels);
//    free(vis_reuslt_h);

    float* vit_pixel_data_d = vit_preprocess(image_base->pixel_data_d, image_base->source_xsize, image_base->source_ysize, vit_size, image_base->channels);
    float* vit_reuslt_h;
    vit_reuslt_h = (float*)malloc(vit_size * vit_size * image_base->channels * sizeof(float));
    cudaMemcpy(vit_reuslt_h, vit_pixel_data_d, vit_size * vit_size * image_base->channels * sizeof(float), cudaMemcpyDeviceToHost);
    cudaFree(vit_pixel_data_d);
    std::string vit_output_filename = output_filename_prefix + "_vit.npy";
    to_npy(vit_output_filename, vit_reuslt_h, vit_size, image_base->channels);
    free(vit_reuslt_h);

    cudaFree(image_base->pixel_data_d);
